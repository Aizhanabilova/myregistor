...
<section class="container page">
	...
	// $avatar = mysqli_query($con, 
	// "SELECT user_avatar FROM users WHERE id=$id");
	?>

	<div class="page-info">
		<div class="user-profile">
		<?php
		if($_SESSION['nickname'] == $_GET['nickname'] && isset($row_user['user_avatar'])){
		?>
			<img class="user-profile--ava" src="<?=$BASE_URL?>/<?=$row_user['user_avatar']?>" alt="">
		<?php
			}else{
			?>
			<img class="user-profile--ava" src="<?=$BASE_URL?>/images/avatar/incognito.svg" alt="">
			<?php
			}
			?>
			...
			<a href="<?=$BASE_URL?>/editindex.php?id=<?=$_SESSION['id']?>" class="button">Редактировать</a>
