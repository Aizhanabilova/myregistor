...
strlen($_POST['email']) > 0){
    ...
    strlen($_FILES['image']['name']) > 0){
        ...
        if(mysqli_num_rows($query_img) > 0){
            ...
            $old_path = __DIR__."../../".$row['user_avatar'];
        ...
        "../../images/avatar/$image_name");
        $path = "images/avatar/$image_name";
        
    $prep = mysqli_prepare($con, 
    "UPDATE users SET nickname=?, full_name=?, email=?, about=?, user_avatar=? WHERE id=?");
